...
const I18N_LEX = {
  ru: {
    ...
    words: {
      ...
      'aizņemta': 'занят', 'kārtotas': 'сортировка', 'nosaukums': 'название', 'augoši': 'по возрастанию',
  ...
  en: {
    ...
    words: {
      ...
      'aizņemta': 'taken', 'kārtotas': 'sorted', 'nosaukums': 'title', 'augoši': 'ascending', 'dilstoši': 'descending',
...
/* Frāžu likumi saliktiem teikumiem (dinamiski skaitļi, laiki, saraksti).
   Likumi tiek apvilkti ar Unicode robežām, tāpēc \b šeit nav vajadzīgs. */
const I18N_PHRASES = {
  ru: [
    [/Šodien/g, 'Сегодня'], [/Rīt/g, 'Завтра'],
    [/Rezervācija spēkā/g, 'Бронь действует'],
    [/vienā pirkumā līdz/g, 'в одной покупке до'], [/vietām/g, 'мест'],
    [/kad laiks beidzas, pasūtījums tiek anulēts un vietas atbrīvotas/g,
      'когда время истёк, заказ аннулируется, а места освобождаются'],
    [/Rezervācijas laiks beidzās/g, 'Время брони истекло'],
    [/Rezervācijas laiks ir beidzies/g, 'Время брони истекло'],
    [/pasūtījums anulēts, izvēlētās vietas atbrīvotas citiem pircējiem/g,
      'заказ аннулирован, выбранные места освобождены для других покупателей'],
    [/vietas atbrīvotas citiem pircējiem/g, 'места освобождены для других покупателей'],
    [/Lūdzu, izvēlieties vietas no jauna/g, 'Пожалуйста, выберите места заново'],
    [/Vienā pirkumā var iegādāties ne vairāk kā/g, 'В одной покупке можно купить не более'],
    [/Sesija beigsies pēc/g, 'Сессия завершится через'],
    [/pirkuma laikā laiks tiek atlikts/g, 'во время покупки время продлевается'],
    [/Cenu filtrs/g, 'Фильтр по цене'], [/redzami/g, 'показано'], [/seansiem/g, 'сеансов'],
    [/Neviena filma neatbilst meklēšanai/g, 'Ни один фильм не соответствует поиску'],
    [/Neviena filma neatbilst izvēlētajiem filtriem/g, 'Ни один фильм не соответствует фильтрам'],
    [/filmas · kārtotas/g, 'фильмов · сортировка'],
    [/Biļete atcelta\. Atmaksāta summa/g, 'Билет отменён. Возвращено'],
    [/Atceltas visas vietas — biļete slēgta\. Atmaksāta summa/g,
      'Отменены все места — билет закрыт. Возвращено'],
    [/Atceltas vietas/g, 'Отменены места'], [/Atmaksāts/g, 'Возвращено'],
    [/Derīgās vietas/g, 'Действующие места'],
    [/Par atceltajām vietām atmaksāts/g, 'За отменённые места возвращено'],
    [/Atzīmējiet vismaz vienu vietu, ko atcelt/g, 'Отметьте хотя бы одно место для отмены'],
    [/Sūtīt saiti uz e-pastu \(simulēts\)/g, 'Отправить ссылку на e-mail (симуляция)'],
    [/Kods darbojas pasūtījumiem no/g, 'Код действует для заказов от'],
    [/Procentu atlaide var būt no 1 līdz 50 %/g, 'Процентная скидка может быть от 1 до 50 %'],
    [/Fiksētā atlaide nevar būt negatīva/g, 'Фиксированная скидка не может быть отрицательной'],
    [/Norādiet filmu, datumu, laiku un cenu/g, 'Укажите фильм, дату, время и цену'],
    [/Norādiet koda nosaukumu un atlaidi/g, 'Укажите название кода и размер скидки'],
    [/Kods «([^»]+)» pievienots/g, 'Код «$1» добавлен'],
    [/netika pievienots/g, 'не добавлен'], [/pievienots/g, 'добавлен'],
    [/Aizņemtās zāles/g, 'Занятые залы'], [/nav izvēlamas/g, 'недоступны для выбора'],
    [/plkst\.\s*/g, 'в '], [/(\d+) no (\d+)/g, '$1 из $2'],
    [/Zālē «([^»]+)» (.+) plkst\. (\d{2}:\d{2}) jau notiek filma «([^»]+)» \((\d{2}:\d{2})\)\. Divi seansi vienā zālē nevar pārklāties\./g,
      'В зале «$1» $2 в $3 уже идёт фильм «$4» ($5). Два сеанса в одном зале не могут пересекаться.'],
    [/Seanss atcelts — izvēlies risinājumu/g, 'Сеанс отменён — выбери решение'],
    [/Kinoteātris atcēla seansu\. Tev ir izvēle: pilna atmaksa vai cita biļete tajā pašā filmā\./g,
      'Кинотеатр отменил сеанс. У тебя есть выбор: полный возврат или другой билет на тот же фильм.'],
    [/Pieprasīt atmaksu/g, 'Запросить возврат'], [/Cita biļete/g, 'Другой билет'],
    [/Izvēlēties citu seansu/g, 'Выбрать другой сеанс'],
    [/Cits šīs filmas seanss šobrīd nav pieejams\./g, 'Другого сеанса этого фильма сейчас нет.'],
    [/Biļete pārcelta/g, 'Билет перенесён'], [/Atmaksa apstiprināta/g, 'Возврат подтверждён'],
    [/Maksājums apstiprināts uzreiz \(([^)]+)\)\./g, 'Платёж подтверждён сразу ($1).'],
    [/Jauna pagaidu parole: ([A-Z0-9-]+)\. Mācību vidē tā tiek parādīta uzreiz\./g,
      'Новый временный пароль: $1. В учебной среде он показывается сразу.'],
    [/Aktīvie seansi/g, 'Активные сеансы'], [/Labot seansu/g, 'Изменить сеанс'],
    [/Atvērta jauna seansa forma ar izvēlētu filmu\./g, 'Открыта форма нового сеанса с выбранным фильмом.'],
    [/Versija/g, 'Версия'], [/Operators: pieslēgts\./g, 'Оператор: подключён.'],
    [/Sākuma stāvoklis/g, 'Исходное состояние'],
    [/Pievienota filma/g, 'Добавлен фильм'], [/Pievienots atlaides kods/g, 'Добавлен код скидки'],
    [/Labots seanss/g, 'Изменён сеанс'], [/Izveidots seanss/g, 'Создан сеанс'],
    [/Atcelts seanss/g, 'Отменён сеанс'], [/Operators pieslēdzās/g, 'Оператор вошёл'],
    [/Atiestatīti dati/g, 'Данные сброшены'], [/bez termiņa/g, 'без срока'],
    [/termiņš/g, 'срок'], [/Atiestatīt/g, 'Сбросить'],
    [/Programmu testēšana/g, 'Тестирование программ'],
    [/dati paliek šajā pārlūkā/g, 'данные остаются в этом браузере'],
    [/mācību vide/g, 'учебная среда']
  ],
  en: [
    [/Šodien/g, 'Today'], [/Rīt/g, 'Tomorrow'],
    [/Rezervācija spēkā/g, 'Booking valid'],
    [/vienā pirkumā līdz/g, 'up to'], [/vietām/g, 'seats'],
    [/kad laiks beidzas, pasūtījums tiek anulēts un vietas atbrīvotas/g,
      'when time runs out the order is cancelled and the seats are released'],
    [/Rezervācijas laiks beidzās/g, 'The booking time has expired'],
    [/Rezervācijas laiks ir beidzies/g, 'The booking time has expired'],
    [/Lūdzu, izvēlieties vietas no jauna/g, 'Please choose seats again'],
    [/Vienā pirkumā var iegādāties ne vairāk kā/g, 'You can buy at most'],
    [/Sesija beigsies pēc/g, 'The session ends in'],
    [/pirkuma laikā laiks tiek atlikts/g, 'time is extended during a purchase'],
    [/Cenu filtrs/g, 'Price filter'], [/redzami/g, 'shown'], [/seansiem/g, 'screenings'],
    [/Neviena filma neatbilst meklēšanai/g, 'No film matches the search'],
    [/Neviena filma neatbilst izvēlētajiem filtriem/g, 'No film matches the filters'],
    [/filmas · kārtotas/g, 'films · sorted'],
    [/Biļete atcelta\. Atmaksāta summa/g, 'Ticket cancelled. Refunded'],
    [/Atceltas vietas/g, 'Cancelled seats'], [/Atmaksāts/g, 'Refunded'],
    [/Derīgās vietas/g, 'Valid seats'],
    [/Par atceltajām vietām atmaksāts/g, 'Refunded for cancelled seats'],
    [/Atzīmējiet vismaz vienu vietu, ko atcelt/g, 'Tick at least one seat to cancel'],
    [/Sūtīt saiti uz e-pastu \(simulēts\)/g, 'Send the link by e-mail (simulated)'],
    [/Kods darbojas pasūtījumiem no/g, 'The code works for orders from'],
    [/Procentu atlaide var būt no 1 līdz 50 %/g, 'A percentage discount can be 1–50%'],
    [/Fiksētā atlaide nevar būt negatīva/g, 'A fixed discount cannot be negative'],
    [/Norādiet filmu, datumu, laiku un cenu/g, 'Enter the film, date, time and price'],
    [/Norādiet koda nosaukumu un atlaidi/g, 'Enter the code name and the discount'],
    [/Kods «([^»]+)» pievienots/g, 'Code “$1” added'],
    [/netika pievienots/g, 'was not added'], [/pievienots/g, 'added'],
    [/Aizņemtās zāles/g, 'Occupied halls'], [/nav izvēlamas/g, 'are not selectable'],
    [/plkst\.\s*/g, 'at '], [/(\d+) no (\d+)/g, '$1 of $2'],
    [/Zālē «([^»]+)» (.+) plkst\. (\d{2}:\d{2}) jau notiek filma «([^»]+)» \((\d{2}:\d{2})\)\. Divi seansi vienā zālē nevar pārklāties\./g,
      'In hall “$1” on $2 at $3 the film “$4” is already running ($5). Two screenings cannot overlap in one hall.'],
    [/Seanss atcelts — izvēlies risinājumu/g, 'Screening cancelled — choose a solution'],
    [/Pieprasīt atmaksu/g, 'Request a refund'], [/Cita biļete/g, 'Another ticket'],
    [/Izvēlēties citu seansu/g, 'Choose another screening'],
    [/Cits šīs filmas seanss šobrīd nav pieejams\./g, 'No other screening of this film right now.'],
    [/Biļete pārcelta/g, 'Ticket moved'], [/Atmaksa apstiprināta/g, 'Refund confirmed'],
    [/Jauna pagaidu parole: ([A-Z0-9-]+)\. Mācību vidē tā tiek parādīta uzreiz\./g,
      'New temporary password: $1. In the training environment it is shown right away.'],
    [/Aktīvie seansi/g, 'Active screenings'], [/Labot seansu/g, 'Edit screening'],
    [/Atvērta jauna seansa forma ar izvēlētu filmu\./g, 'The new screening form is open with a film selected.'],
    [/Versija/g, 'Version'], [/Operators: pieslēgts\./g, 'Operator: signed in.'],
    [/Sākuma stāvoklis/g, 'Initial state'],
    [/Pievienota filma/g, 'Film added'], [/Pievienots atlaides kods/g, 'Discount code added'],
    [/Labots seanss/g, 'Screening edited'], [/Izveidots seanss/g, 'Screening created'],
    [/Atcelts seanss/g, 'Screening cancelled'], [/Operators pieslēdzās/g, 'Operator signed in'],
    [/Atiestatīti dati/g, 'Data reset'], [/bez termiņa/g, 'no expiry'],
    [/termiņš/g, 'expiry'], [/Atiestatīt/g, 'Reset'],
    [/Programmu testēšana/g, 'Software testing'],
    [/dati paliek šajā pārlūkā/g, 'data stays in this browser'],
    [/mācību vide/g, 'training environment']
  ]
};
...
/* BUG-11: pārzīmē tikai zāļu sarakstu un padomu — bez pilnas lapas pārzīmēšanas,
   citādi change notikums iekš render() sabojā DOM (nodaļa tiek pārvietota). */
function refreshHallOptions(){
  const sel=document.querySelector('#sc-hall');
  if(!sel) return;
  const edSc=view.editingScreening?screening(view.editingScreening):null;
  const scr=view.scr||{};
  const movieId=String(scr.movieId||view.screeningMovie||(edSc?edSc.movieId:''));
  const dEl=document.querySelector('#sc-date'), tEl=document.querySelector('#sc-time');
  const date=scr.date||(dEl?dEl.value:'')||(edSc?edSc.date:todayISO());
  const time=scr.time||(tEl?tEl.value:'')||(edSc?edSc.time:'19:00');
  let clashes=0;
  [...sel.options].forEach(o=>{
    const c=screeningConflict({movieId,date,time,hall:o.value},edSc?edSc.id:null);
    o.disabled=!!c;
    o.textContent=o.value+(c?` — aizņemta (${c.time})`:'');
    if(c) clashes++;
  });
  if(sel.options[sel.selectedIndex]&&sel.options[sel.selectedIndex].disabled){
    const free=[...sel.options].find(o=>!o.disabled);
    if(free){sel.value=free.value;view.scr=Object.assign({},view.scr||{},{hall:free.value})}
  }
  const hint=document.querySelector('#hall-hint');
  if(hint) hint.textContent=clashes
    ? `Aizņemtās zāles ${dateLabel(date)} plkst. ${time} nav izvēlamas (${clashes} no ${Object.keys(state.halls).length}).`
    : '';
}
...
function booking(){
  ...
  /* BUG-59: ratiņkrēsla vietas atzīmē gan klase, gan IEKĻAUTAIS stils un ♿ zīme — tā vieta
   ir atpazīstama arī tad, ja pārlūkā ir palicis vecs styles.css no kešatmiņas. */
const seatRows=rows.map(r=>`<div class="seat-row" style="--seats:${hall.seats}"><span class="row-label">${r}</span>${Array.from({length:hall.seats},(_,i)=>{let n=`${r}${i+1}`,taken=reserved(s.id).includes(n),selected=view.seats.includes(n),wheel=inv.includes(n);
    const inline=wheel?` style="background:${selected?'#ff8d32':'#a9d3f0'};border:2px dashed ${selected?'#11131a':'#6fa8dc'};color:${selected?'#11131a':'#12283a'}"`:'';
    return `<button class="seat ${taken?'taken':''} ${selected?'selected':''} ${wheel?'wheel':''}" data-seat="${n}" ${taken?'disabled':''}${wheel?' title="Invalīdu vieta '+n+'"':''}${inline}>${wheel?'♿':''}${i+1}</button>`}).join('')}</div>`).join('');
...
function admin(){
  ...
  /* BUG-11: formas vērtības (datums/laiks/zāle) — no tām atkarīgs, kuras zāles ir aizņemtas. */
  const scr=view.scr||{};
  const formMovie=String(scr.movieId||preMovie||'');
  const formDate=scr.date||(edSc?edSc.date:isoDate(0));
  const formTime=scr.time||(edSc?edSc.time:'19:00');
  const formHall=scr.hall||(edSc?edSc.hall:'Zāle 1');
  const hallClashes=Object.keys(state.halls).filter(h=>screeningConflict({movieId:formMovie,date:formDate,time:formTime,hall:h},edSc?edSc.id:null)).length;
  ...
  <div class="form-actions"><button class="button secondary" data-action="add-promo">Pievienot kodu</button></div>
  ${view.promoMsg?notice(view.promoMsg,view.promoMsgType||'success'):''}</div>
  ...
  <label>Datums<input id="sc-date" type="date" value="${formDate}"></label>
  <label>Laiks<input id="sc-time" type="time" value="${formTime}"></label>
  <label>Zāle<select id="sc-hall">${Object.keys(state.halls).map(h=>{const clash=screeningConflict({movieId:formMovie,date:formDate,time:formTime,hall:h},edSc?edSc.id:null);return `<option value="${h}"${formHall===h?' selected':''} ${clash?'disabled':''}>${h}${clash?` — aizņemta (${clash.time})`:''}</option>`}).join('')}</select></label>
  <label>Cena €<input id="sc-price" type="number" min="1" step="0.5" value="${edSc?edSc.price:8.5}"></label></div>
  <p class="meta" id="hall-hint">${hallClashes?`Aizņemtās zāles ${dateLabel(formDate)} plkst. ${formTime} nav izvēlamas (${hallClashes} no ${Object.keys(state.halls).length}).`:''}</p>
...
document.addEventListener('click',e=>{
  ...
  if(b.dataset.action==='new-screening'){
    view.screeningMovie=Number(b.dataset.id);view.editingScreening=null;view.scr=null;
    view.notice='Atvērta jauna seansa forma ar izvēlētu filmu.';return render();
  }
  if(b.dataset.action==='edit-screening'){
    view.editingScreening=Number(b.dataset.id);view.screeningMovie=null;view.scr=null;view.notice=null;return render();
  }
  ...
  if(b.dataset.action==='add-promo'){
    ...
    if(!code||!val){view.promoMsg='Norādiet koda nosaukumu un atlaidi.';view.promoMsgType='error';return render()}
    /* BUG-12 / BUG-13 (US-07.03): maksimālā atlaide ir 50 % — lielāku kodu sistēma nepieņem. */
    if(kind==='percent'&&(val<1||val>50)){
      view.promoMsg=`Procentu atlaide var būt no 1 līdz 50 %. Kods «${code}» netika pievienots.`;
      view.promoMsgType='error';toast(view.promoMsg);
      return render();
    }
    if(kind==='eur'&&val<0){
      view.promoMsg='Fiksētā atlaide nevar būt negatīva.';view.promoMsgType='error';toast(view.promoMsg);
      return render();
    }
    state.promos[code]={kind,value:val,min,until};
    view.promoMsg=`Kods «${code}» pievienots: ${kind==='eur'?`−${money(val)}`:`−${val}%`}${min?` · no ${money(min)}`:''}${until?` · termiņš ${until}`:''}.`;
    view.promoMsgType='success';toast(view.promoMsg);
...
document.addEventListener('change',e=>{
  ...
  if(['sc-movie','sc-date','sc-time','sc-hall','sc-price'].includes(e.target.id)){
    const key=e.target.id.slice(3), map={movie:'movieId',date:'date',time:'time',hall:'hall',price:'price'};
    view.scr=Object.assign({},view.scr||{},{[map[key]]:e.target.value});
    refreshHallOptions();
    return;
  }
